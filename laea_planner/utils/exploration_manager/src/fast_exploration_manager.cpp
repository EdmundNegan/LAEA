...
namespace fast_planner
{
      ...
      const Vector3d &pos, const Vector3d &vel, const Vector3d &acc, const Vector3d &yaw)
  {
    ...
    for (int attempt = 0; attempt < max_attempts; ++attempt)
    {
      ...
      ROS_WARN("Attempt %d: No path to next viewpoint from (%.2f, %.2f, %.2f) to (%.2f, %.2f, %.2f)",
               attempt + 1, pos.x(), pos.y(), pos.z(), next_pos.x(), next_pos.y(), next_pos.z());
    }

    if (!path_found)
    {
      ROS_ERROR("No path to next viewpoint after %d attempts", max_attempts);
      // Optionally, implement a fallback strategy here
      return FAIL;
    }
